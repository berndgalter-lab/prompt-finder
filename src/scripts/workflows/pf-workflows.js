...
/**
 * Update Master Progress Bar (overall completion)
 * Calculates total progress across workflow + all step variables
 */
function updateMasterProgress() {
  // Get master progress data from JSON (generated by PHP)
  const dataScript = document.getElementById('pf-master-progress-data');
  if (!dataScript) return; // No master progress on this page
  
  let progressData;
  try {
    progressData = JSON.parse(dataScript.textContent);
  } catch (e) {
    console.error('Failed to parse master progress data:', e);
    return;
  }
  
  const totalVars = progressData.totalVars || 0;
  if (totalVars === 0) return; // No variables to track
  
  // Count filled variables across ALL sections (WF + Steps)
  const allVars = document.querySelectorAll('.pf-var[data-field-name]');
  let totalFilled = 0;
  
  // Track per-section counts for status icons
  const sectionCounts = {};
  
  allVars.forEach(varEl => {
    const fieldName = varEl.getAttribute('data-field-name');
    const variableTier = varEl.getAttribute('data-variable-tier'); // 'workflow' or 'step'
    ...
    if (input) {
      ...
      // Track per-section (for status icons)
      const sectionKey = variableTier === 'workflow' ? 'workflow' : variableTier;
      if (!sectionCounts[sectionKey]) {
        sectionCounts[sectionKey] = { total: 0, filled: 0 };
      }
      sectionCounts[sectionKey].total++;
      if (isFilled) {
        sectionCounts[sectionKey].filled++;
      }
    }
  });
  
  // Calculate percentage
  const percentage = totalVars > 0 ? Math.round((totalFilled / totalVars) * 100) : 0;
  
  // Update Master Progress UI
  const filledEl = document.querySelector('[data-master-filled]');
  const totalEl = document.querySelector('[data-master-total]');
  const percentageEl = document.querySelector('[data-master-percentage]');
  const progressFill = document.querySelector('[data-master-progress-fill]');
  const progressBar = document.querySelector('.pf-master-progress-bar');
  
  if (filledEl) filledEl.textContent = totalFilled;
  if (totalEl) totalEl.textContent = totalVars;
  ...
  updateSectionStatusIcons(sectionCounts);
}

/**
 * Update status icons for each section (workflow + steps)
 * Shows: ○ pending, ⚠️ partial, ✓ complete
 */
function updateSectionStatusIcons(sectionCounts) {
  const sections = document.querySelectorAll('.pf-progress-section');
  
  sections.forEach(section => {
    const sectionKey = section.getAttribute('data-section');
    const statusIcon = section.querySelector('[data-section-status]');
    
    if (!statusIcon || !sectionKey) return;
    
    const counts = sectionCounts[sectionKey];
    
    if (!counts || counts.total === 0) {
      // No variables in this section (shouldn't happen, but handle gracefully)
      setStatusIcon(statusIcon, 'pending');
      section.removeAttribute('data-section-complete');
      return;
    }
    
    const sectionPercentage = (counts.filled / counts.total) * 100;
    
    // Update section's data attributes
    section.setAttribute('data-section-total', counts.total);
    section.setAttribute('data-section-filled', counts.filled);
    
    // Determine status
    if (sectionPercentage === 0) {
      // Nothing filled - Pending
      setStatusIcon(statusIcon, 'pending');
      section.removeAttribute('data-section-complete');
    } else if (sectionPercentage === 100) {
      // Everything filled - Complete
      setStatusIcon(statusIcon, 'complete');
      section.setAttribute('data-section-complete', 'true');
    } else {
      // Some filled - Partial
      setStatusIcon(statusIcon, 'partial');
      section.setAttribute('data-section-complete', 'partial');
    }
  });
}

/**
 * Helper: Set status icon (pending/partial/complete)
 */
function setStatusIcon(statusElement, status) {
  const iconEl = statusElement.querySelector('.pf-status-icon');
  if (!iconEl) return;
  
  // Remove all status classes
  iconEl.classList.remove('pf-status-icon--pending', 'pf-status-icon--partial', 'pf-status-icon--complete');
  
  // Add new status class
  iconEl.classList.add(`pf-status-icon--${status}`);
  
  // Update SVG content based on status
  if (status === 'pending') {
    // Empty circle
    iconEl.innerHTML = '<circle cx="12" cy="12" r="10"></circle>';
    ...
    // Checkmark
    iconEl.innerHTML = '<polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path>';
  }
}
